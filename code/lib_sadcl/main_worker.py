...
from models.sadcl import build_sadcl
from utils.misc import clean_state_dict, init_distributed_and_seed
from utils.util import add_weight_decay, ModelEma, save_checkpoint, kill_process, load_model, WarmUpLR, cal_gpu
...
def main_worker(args, logger):
    ...
    # criterion
    criterion = {}
    # criterion['aslloss'] = torch.nn.MultiLabelSoftMarginLoss()
    ...
    criterion['contrasloss'] = models.loss.multisupconloss.MultiSupConLoss()
    criterion['sem2pro'] = models.loss.sem2pro.Semantic2PrototypeConLoss()
    ...
    if args.evaluate:
        _, _, _, mAP = validate(val_loader, model, criterion, args, logger)
    ...
    losses_bce = AverageMeter('Loss_bce', ':5.3f', val_only=True)
    losses_bce_ema = AverageMeter('Loss_bce_ema', ':5.3f', val_only=True)
    losses_contras = AverageMeter('Loss_contras', ':5.3f', val_only=True)
    losses_contras_ema = AverageMeter('Loss_contrast_ema', ':5.3f', val_only=True)
    mAPs = AverageMeter('mAP', ':5.5f', val_only=True)
    mAPs_ema = AverageMeter('mAP_ema', ':5.5f', val_only=True)
    progress = ProgressMeter(
        args.epochs,
        [eta, epoch_time, losses, losses_bce, losses_contras, mAPs, losses_ema, losses_bce_ema, losses_contras_ema, mAPs_ema],
        prefix='=> Test Epoch: ')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss, loss_bce, loss_contras = train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger, warmup_scheduler)

        if summary_writer:
            # tensorboard logger
            summary_writer.add_scalar('train_loss', loss, epoch)
            summary_writer.add_scalar('train_loss_bce', loss_bce, epoch)
            summary_writer.add_scalar('train_loss_contras', loss_contras, epoch)
            summary_writer.add_scalar('learning_rate', optimizer.param_groups[0]['lr'], epoch)

        if epoch % args.val_interval == 0:

            # evaluate on validation set
            loss, loss_bce, loss_contras, mAP = validate(val_loader, model, criterion, args, logger)
            loss_ema, loss_bce_ema, loss_contras_ema, mAP_ema = validate(val_loader, ema_m.module, criterion, args, logger)

            losses.update(loss)
            losses_bce.update(loss_bce)
            losses_contras.update(loss_contras)
            mAPs.update(mAP)

            losses_ema.update(loss_ema)
            losses_bce_ema.update(loss_bce_ema)
            losses_contras_ema.update(loss_contras_ema)
            ...
            if summary_writer:
                ...
                summary_writer.add_scalar('val_loss_bce', loss_bce, epoch)
                summary_writer.add_scalar('val_loss_contras', loss_contras, epoch)
                summary_writer.add_scalar('val_mAP', mAP, epoch)

                summary_writer.add_scalar('val_loss_ema', loss_ema, epoch)
                summary_writer.add_scalar('val_loss_bce_ema', loss_bce_ema, epoch)
                summary_writer.add_scalar('val_loss_contras_ema', loss_contras_ema, epoch)
